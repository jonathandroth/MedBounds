...
test_sharp_null <- function(df,
  ...
  # eta: nuisance par to create target par
  ...
  A.shp <- matrix(0, nrow = length(l_gt_k_inds) + K, ncol = len_x)
  A.shp[cbind(1:length(l_gt_k_inds), l_gt_k_inds)] <- 1

  # Set bound on sum_{l!=k} theta_lk - sum_q delta_qk
  for (k in 1:K) {
    A.shp[length(l_gt_k_inds) + k, ((k-1) * K + 1):(k * K)] <- 1
    A.shp[length(l_gt_k_inds) + k, (k-1) * K + k] <- 0
    A.shp[length(l_gt_k_inds) + k, par_lengths[1] + ((k-1) * d_y + 1):(k * d_y)] <- -1
    
    A.shp[length(l_gt_k_inds) + k, sum(par_lengths[1:2]) + k] <- 1
  }
  
  beta.shp <- rep(0, length(l_gt_k_inds) + K)


  # Set remaining constraints using A.obs x = beta.obs

  # Constructing beta_obs
  # WARNING: table may omit rows/columns with 0 observations; to be fixed later
  ...
  for (k in 1:K) { 
    ...
    # Nuisance par to create target
    A.obs[k, sum(par_lengths[1:4]) + k] <- 1
  }
